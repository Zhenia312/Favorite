...
async def check_pending_results(session):
    try:
        conn = sqlite3.connect(DB_PATH)
        rows = conn.execute(
            "SELECT id, fixture_id, signal_type, fav_side, edge_side, score_at_signal FROM signals WHERE status='pending'"
        ).fetchall()
        conn.close()
    except Exception as e:
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        updated = 0
        for row_id, fixture_id, signal_type, fav_side, edge_side, score_at_signal in rows:
