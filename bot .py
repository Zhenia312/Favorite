...
def init_db():
    try:
        ...
        #
        # ── МІГРАЦІЯ (доповнення показників): confidence + сира статистика
        # (удари всього, удари в створ, кутові, червоні картки) для фаворита
        # й андердога на момент сигналу. Це ДОДАТКОВІ поля, які лише
        # підсилюють/маркують уже існуючі сигнали (confidence), і НІЧОГО
        # не блокують — навіть червона картка є лише підсиленням, а не
        # причиною відмінити сигнал.
        for col, col_type in [
            ("edge_side", "TEXT"), ("edge_result", "TEXT"),
            ("confidence", "TEXT"),
            ("shots_total_fav", "INTEGER"), ("shots_total_und", "INTEGER"),
            ("sot_fav", "INTEGER"), ("sot_und", "INTEGER"),
            ("corners_fav", "INTEGER"), ("corners_und", "INTEGER"),
            ("red_fav", "INTEGER"), ("red_und", "INTEGER"),
        ]:
            try:
...
def save_signal(fixture_id, signal_type, league, fav_team, und_team, fav_side,
                 ...
                 edge_side=None, confidence=None, match_stats=None):
    """
    match_stats — опційний dict з полями fav_sot/und_sot/fav_shots_total/
    und_shots_total/fav_corners/und_corners/fav_red/und_red (див.
    get_fav_und_stats). Якщо None — відповідні колонки лишаються NULL
    (наприклад, коли статистику не вдалось отримати від API).
    """
    ms = match_stats or {}
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute("""
            INSERT INTO signals (
                ...
                pre_odd, live_odd, rise_pct, score_at_signal, minute_at_signal,
                signal_time, status, edge_side, confidence,
                shots_total_fav, shots_total_und, sot_fav, sot_und,
                corners_fav, corners_und, red_fav, red_und
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            fixture_id, signal_type, league, fav_team, und_team, fav_side,
            pre_odd, live_odd, rise_pct, score_at_signal, minute_at_signal,
            now_kyiv().strftime("%Y-%m-%d %H:%M:%S"), edge_side, confidence,
            ms.get("fav_shots_total"), ms.get("und_shots_total"),
            ms.get("fav_sot"), ms.get("und_sot"),
            ms.get("fav_corners"), ms.get("und_corners"),
            ms.get("fav_red"), ms.get("und_red"),
        ))
        conn.commit()
        conn.close()
    except Exception as e:
...
async def send_results_stat(session):
    try:
        ...
        # ── розбивка по confidence: дозволяє порівняти winrate "standard"
        # проти "confirmed"/"high" (усилених ударами/кутовими/картками)
        conf_rows = conn.execute(
            "SELECT confidence, result FROM signals WHERE status='completed' AND confidence IS NOT NULL"
        ).fetchall()
        conn.close()
    except Exception as e:
    ...
    by_conf = {}
    for conf, res in conf_rows:
        d = by_conf.setdefault(conf, {"win": 0, "total": 0})
        d["total"] += 1
        if res == "win":
            d["win"] += 1
    ...
    if by_conf:
        lines.append("\n*По рівню confidence (удари/кутові/картки):*")
        conf_order = {"high": 0, "confirmed": 1, "standard": 2}
        for conf, d in sorted(by_conf.items(), key=lambda x: conf_order.get(x[0], 9)):
            c_pct = round(d["win"] / d["total"] * 100, 1) if d["total"] else 0
            lines.append(f"  - {escape_md(conf)}: {d['win']}/{d['total']} ({c_pct}%)")
    ...
    try:
        ...
        cur = conn.execute("""
            SELECT id, fixture_id, signal_type, league, fav_team, und_team, fav_side,
                   ...
                   confidence, shots_total_fav, shots_total_und, sot_fav, sot_und,
                   corners_fav, corners_und, red_fav, red_und
            FROM signals
            ORDER BY id ASC
        """)
...
# ── ДОДАТКОВІ ПОКАЗНИКИ ЯК ПІДСИЛЕННЯ ІСНУЮЧИХ СИГНАЛІВ ────────────────────
# Удари всього / в створ / кутові / червоні картки НЕ створюють новий тип
# сигналу і НІЧОГО не блокують (включно з червоною карткою — вона лише
# підсилює confidence, ніколи не скасовує сигнал). Кожен з чотирьох
# сигналів (strong_cant_win, fav_losing, no_goals, sot_total_high)
# отримує поле confidence: "standard" (базові умови) / "confirmed"
# (підтверджено статистикою) / "high" (сильне подвійне підтвердження).
STRONG_CANT_WIN_SOT_DOMINANCE_RATIO = 2.0  # fav_sot >= und_sot * ratio → підтверджено
LOSING_CORNERS_CONFIRM_DIFF         = 2    # fav_corners - und_corners >= X → підтверджено
NO_GOALS_SHOTS_CONFIRM_MAX          = 6    # сумарні удари всього <= X підтверджують "мертву" гру
...
def confidence_label(confidence):
    return {
        ...
        "standard":  "◻️ Базова",
    }.get(confidence, "◻️ Базова")

# ── СТАТИСТИКА МАТЧУ (удари всього/в створ, кутові, червоні картки) ────────
# Один запит до /fixtures/statistics покриває ВСІ чотири показники одразу.
# Викликається ТІЛЬКИ для кандидатів, що вже пройшли базовий фільтр
# (рахунок+коефіцієнт+хвилина) — щоб не витрачати квоту на всі live-матчі
# підряд. З квотою 7500/день це безпечно.
async def fetch_fixture_statistics(session, fixture_id):
    """
    Повертає dict {
        "shots_on_target_home": int, "shots_on_target_away": int,
        "shots_total_home": int, "shots_total_away": int,
        ...
        async with session.get(
            ...
            def extract(team_block):
                ...
                for stat in team_block.get("statistics", []):
                    ...
                    val   = stat.get("value")
                    if stype == "shots on goal":
                        shots_on_target = to_int(val)
                    elif stype == "total shots":
                        shots_total = to_int(val)
                    elif stype == "corner kicks":
                        corners = to_int(val)
                    elif stype == "red cards":
                        red_cards = to_int(val)
                return shots_on_target, shots_total, corners, red_cards

            # API зазвичай повертає [0]=home, [1]=away, але team.id
            # не завжди доступний у відповіді — припускаємо порядок home/away
            home_sot, home_total, home_corners, home_red = extract(data[0])
            away_sot, away_total, away_corners, away_red = extract(data[1])

            return {
                "shots_on_target_home": home_sot,
                "shots_on_target_away": away_sot,
                "shots_total_home": home_total,
                "shots_total_away": away_total,
...
def get_fav_und_stats(stats_data, fav_side):
    """
    Розкладає сиру статистику матчу (home/away) по ролях фаворит/андердог.
    Повертає None, якщо stats_data недоступна (наприклад, ліга без покриття
    статистики) — у цьому разі confidence лишається "standard" за замовчуванням,
    а сигнал ВСЕ ОДНО відправляється (відсутність статистики нічого не блокує).
    """
    if stats_data is None:
        return None
    if fav_side == "home":
        return {
            "fav_sot":         stats_data["shots_on_target_home"],
            "und_sot":         stats_data["shots_on_target_away"],
            "fav_shots_total": stats_data["shots_total_home"],
            "und_shots_total": stats_data["shots_total_away"],
            "fav_corners":     stats_data["corners_home"],
            "und_corners":     stats_data["corners_away"],
            "fav_red":         stats_data["red_cards_home"],
            "und_red":         stats_data["red_cards_away"],
        }
    return {
        "fav_sot":         stats_data["shots_on_target_away"],
        "und_sot":         stats_data["shots_on_target_home"],
        "fav_shots_total": stats_data["shots_total_away"],
        "und_shots_total": stats_data["shots_total_home"],
        "fav_corners":     stats_data["corners_away"],
        "und_corners":     stats_data["corners_home"],
        "fav_red":         stats_data["red_cards_away"],
        "und_red":         stats_data["red_cards_home"],
    }

def build_stats_line(ms, fav_team, und_team):
    """Рядок для повідомлення в Telegram з ударами/кутовими/картками (якщо є дані)."""
    if ms is None:
        return ""
    parts = [
        f"Удари всього: {fav_team} {ms['fav_shots_total']} — {und_team} {ms['und_shots_total']}",
        f"Удари в створ: {fav_team} {ms['fav_sot']} — {und_team} {ms['und_sot']}",
        f"Кутові: {fav_team} {ms['fav_corners']} — {und_team} {ms['und_corners']}",
    ]
    if ms["fav_red"] > 0 or ms["und_red"] > 0:
        parts.append(f"🟥 Червоні картки: {fav_team} {ms['fav_red']} — {und_team} {ms['und_red']}")
    return "\n".join(parts) + "\n"

# ── CONFIDENCE ДЛЯ КОЖНОГО ТИПУ СИГНАЛУ ────────────────────────────────────
# Червона картка НІКОЛИ не блокує сигнал — вона лише піднімає confidence,
# коли грає на користь напряму сигналу (червона в андердога підтверджує,
# що фаворит виправдано вважається сильнішим "по кадрам").
def confidence_strong_cant_win(ms):
    if ms is None:
        return "standard"
    if ms["und_red"] > 0:
        return "high"
    if ms["fav_sot"] >= ms["und_sot"] * STRONG_CANT_WIN_SOT_DOMINANCE_RATIO and ms["und_sot"] > 0 or \
       (ms["und_sot"] == 0 and ms["fav_sot"] >= 2):
        return "confirmed"
    return "standard"

def confidence_fav_losing(ms):
    if ms is None:
        return "standard"
    if ms["und_red"] > 0 and (ms["fav_corners"] - ms["und_corners"]) >= LOSING_CORNERS_CONFIRM_DIFF:
        return "high"
    if ms["und_red"] > 0:
        return "confirmed"
    if (ms["fav_corners"] - ms["und_corners"]) >= LOSING_CORNERS_CONFIRM_DIFF:
        return "confirmed"
    return "standard"

def confidence_no_goals(ms):
    if ms is None:
        return "standard"
    total_shots = ms["fav_shots_total"] + ms["und_shots_total"]
    if total_shots <= NO_GOALS_SHOTS_CONFIRM_MAX:
        return "confirmed"
    return "standard"
...
async def _process_football_fixtures(session, fixtures):
    ...
    for fix in fixtures:
        ...
        try:
            ...
            # ── ДОДАТКОВІ ПОКАЗНИКИ: один запит статистики на кандидата ─────
            # Покриває удари всього/в створ, кутові, червоні картки одразу.
            # Використовується і для confidence основного сигналу (нижче),
            # і повторно для sot_total_high (без другого запиту до API).
            main_stats_data = await fetch_fixture_statistics(session, fid)
            ms = get_fav_und_stats(main_stats_data, fav_side)
            stats_line = build_stats_line(ms, fav_team, und_team)
            ...
            if TEST_MODE and is_losing_case:
                ...
                if test_skip_reason:
                    ...
                    # статистику матчу (удари в створ, червоні картки), яку
                    # вже отримали вище як main_stats_data/ms.
                    shots_fav = ms["fav_sot"] if ms else None
                    shots_und = ms["und_sot"] if ms else None
                    red_und   = ms["und_red"] if ms else 0
            ...
            if is_losing_case:
                ...
                await send_msg(session,
                    ...
                    f"Коеф зараз: {live_odd} (+{rise}%)\n"
                    f"{escape_md(stats_line)}"
                    f"💪 {strength(rise)} | Confidence: {confidence_label(confidence)}"
                )
                print(f"  ⚽ СИГНАЛ fid={fid}: {fav_team} програє {fav_score}:{und_score} +{rise}% conf={confidence}")

            elif is_no_goals_case:
                confidence = confidence_no_goals(ms)
                ...
                print(f"  ⚽ СИГНАЛ fid={fid} тотал<1.75: {fav_team} vs {und_team} {minute}' +{rise}% conf={confidence}")

            elif is_strong_cant_win:
                confidence = confidence_strong_cant_win(ms)
                ...
                print(f"  ⚽ СИГНАЛ fid={fid} сильний фаворит не виграє: {fav_team} {score_h}:{score_a} +{rise}% conf={confidence}")
            ...
            # Не блокує і не замінює основний сигнал. Використовує ту саму
            # main_stats_data/ms, що вже отримали вище — другого запиту
            # до /fixtures/statistics не робимо.
            if sot_candidate and key_sot_total not in notified:
                if ms is not None:
                    shots_fav = ms["fav_sot"]
                    shots_und = ms["und_sot"]
                    ...
                    if shots_total >= SOT_TOTAL_MIN_SHOTS:
                        ...
                        save_signal(fid, "sot_total_high", league_name, fav_team, und_team, fav_side,
                                    fav_odd, live_odd, rise, f"{score_h}:{score_a}", minute,
                                    edge_side=edge_side, match_stats=ms)
                        await send_msg(session,
